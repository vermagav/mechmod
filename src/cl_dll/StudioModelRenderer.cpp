...
rendercheck_t g_RenderCheck[33];

// Pcjoe: Fly Animation Order
typedef enum
{	ANIM_LEFT=0,
	ANIM_RIGHT,
	ANIM_UP,
...
// Pcjoe: Needed so we can use the vector angles function
extern void VectorAngles( const float *forward, float *angles );

// Pcjoe: Velocty of player stored from pm_shared.c
extern "C"
{	vec3_t vecPmVelocity;
};
/////////////////////
...
//			Con_DPrintf("%.0f %.0f\n",m_pCurrentEntity->msg_angles[0][YAW], m_pCurrentEntity->msg_angles[1][YAW] );
	...
	// Pcjoe: Copy over angles
	std::list<lockmdl_t>::iterator iterCurrent;
	ForEachIn(iterCurrent,gHUD.m_LockMdl.m_LockEnts)
	{	// Matching locked model
		if(iterCurrent->iIndex == m_pCurrentEntity->index)
		{	angles = iterCurrent->vecAng;
			break;
		}
	}
...
====================
*/
vec3_t CStudioModelRenderer::GetVelocity( cl_entity_t *pEntity )
{	if(!pEntity)
	{	return vec3_t(0,0,0);
	}

	if(pEntity == gEngfuncs.GetLocalPlayer())
	{	return vecPmVelocity;
	}
	else
	{	return (( m_pCurrentEntity->latched.prevorigin-m_pCurrentEntity->origin) * (1/(m_clTime - m_clOldTime)));
	}
	return vec3_t(0,0,0);
}
...
void CStudioModelRenderer::StudioSetupBones ( void )
{
	...
	if (m_pPlayerInfo && m_pPlayerInfo->gaitsequence != 0)
	{
		...
		// Added by Pcjoe, smooth gait anim interpolation
		int iMaxFrames = pseqdesc->numframes;
		if(m_pCurrentEntity->curstate.movetype == MOVETYPE_PLAYERFLY)
		{	
			// modifed by jason
			pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + GetSequenceNumber("fly_idle");

			panim = StudioGetAnim( m_pRenderModel, pseqdesc );
			StudioCalcRotations( pos2, q2, pseqdesc, panim, m_pPlayerInfo->gaitframe);


			static float		pos5[MAXSTUDIOBONES][3];
			static vec4_t		q5[MAXSTUDIOBONES];
			float s;
			float flZ = 0;
			int iMaxSpeed;
			
			// Get max speed
			iMaxSpeed = m_pCurrentEntity->curstate.iuser4;

			// Get interpolation from flyidle to moving anims
			vec3_t vecVel = GetVelocity(m_pCurrentEntity);
			...
			vec3_t v_forward;
			AngleVectors(m_pCurrentEntity->angles, v_forward, NULL, NULL);

			VectorNormalize(vecVel);

			vec3_t vecAngles;
			VectorAngles(vecVel, vecAngles);
			float flAngles = m_pCurrentEntity->angles.y;
			if(flAngles < 0)
				flAngles += 360;

			flAngles -= vecAngles.y;
			float flAngles2 = flAngles;
			if(flAngles < 0)
				flAngles += 360;

			int iAnim1=0, iAnim2=0;
			float flRatio = 0;

			// Front - right
			if(flAngles < 90)
			{	flRatio = flAngles / float(90);
				iAnim1 = ANIM_FRONT;
				iAnim2 = ANIM_RIGHT;
			}
			// right - back
			else if(flAngles < 180)
			{	flAngles -= 90;
				flRatio = flAngles / float(90);
				iAnim1 = ANIM_RIGHT;
				iAnim2 = ANIM_BACK;
			...
			int iFlyBend = GetSequenceNumber("fly_left");

			// Anim 1
			pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + (iFlyBend+iAnim1);
			panim = StudioGetAnim( m_pRenderModel, pseqdesc );
			StudioCalcRotations( pos3, q3, pseqdesc, panim, m_pPlayerInfo->gaitframe/iMaxFrames * pseqdesc->numframes );

			// Anim 2
			pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + (iFlyBend+iAnim2);
			panim = StudioGetAnim( m_pRenderModel, pseqdesc );
			StudioCalcRotations( pos4, q4, pseqdesc, panim, m_pPlayerInfo->gaitframe/iMaxFrames * pseqdesc->numframes );

			flZ = fabs(vecVel.z);

			// Anim 3 ( up )
			if(vecVel.z > 0)
				pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + (iFlyBend+ANIM_UP);
			// Anim 3 ( down )
			else
				pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + (iFlyBend+ANIM_DOWN);

			panim = StudioGetAnim( m_pRenderModel, pseqdesc );
			StudioCalcRotations( pos5, q5, pseqdesc, panim, m_pPlayerInfo->gaitframe/iMaxFrames * pseqdesc->numframes );

			// Slerp
			StudioSlerpBones( q3, pos3, q4, pos4, flRatio );	// Horizontal aniamtions
			StudioSlerpBones( q3, pos3, q5, pos5, flZ );		// Up animation
			StudioSlerpBones( q2, pos2, q3, pos3, s );			// Blend everything together
			
			// Copy wing bone information ( only for freedom gundam )
		/*	if(!stricmp(m_pRenderModel->name, "models/player/freedom/freedom.mdl") )
			{
				if( strName.find( "5wayattack" ) == std::string::npos )
				{
					std::string strBone;
					for (i = 0; i < m_pStudioHeader->numbones; i++)
					{	strBone = pbones[i].name;
						if(strBone.find("wing") != -1)
		...
		for (i = 0; i < m_pStudioHeader->numbones; i++)
		{
			if (stricmp( pbones[i].name, "Bip01 Spine") == 0)
				break;
			memcpy( pos[i], pos2[i], sizeof( pos[i] ));
			memcpy( q[i], q2[i], sizeof( q[i] ));
		}
...
====================
*/
void CStudioModelRenderer::StudioSaveBones( int &flags )
{
	int		i, n;
	// Added by Pcjoe
	bool	first = false;

	recboneiter iterRecBones = gHUD.m_BonePos.FindBoneRecord(m_pCurrentEntity->index);
	if(iterRecBones != NULL)
	{	// Rendered this frame
		iterRecBones->blRenderedFrame = true;
		// Set it so other code structures can use bone origins
		// modifed by jason
		if( m_pCurrentEntity->player )
		{
			if(!iterRecBones->blCanUse && m_pPlayerInfo )
			{	iterRecBones->blCanUse = true;
				first = true;
				iterRecBones->iMaxBones = m_pStudioHeader->numbones;
			}
		}
		else if( gEngfuncs.GetEntityByIndex( m_pCurrentEntity->index ) != m_pCurrentEntity )
		{
			// hopefully the weapon since we dont want weapons
			iterRecBones = NULL;
			// attachments
		}
		else
		{
			// nonplayer non attachments
			if(!iterRecBones->blCanUse )
			{	iterRecBones->blCanUse = true;
				first = true;
				iterRecBones->iMaxBones = m_pStudioHeader->numbones;
			}
		...
		// Disable rendering if specified
		if(!iterRecBones->blRender)
		{	flags &= ~STUDIO_RENDER;
		}
	}
	...
	for (i = 0; i < m_pStudioHeader->numbones; i++) 
	{
		...
		// Pcjoe: Record bone origins
		if(iterRecBones != NULL)
		{	// Only need to copy the name / parent bone once
			if(first)
			{	strcpy( iterRecBones->szName[i], pbones[i].name );
				iterRecBones->iParent[i] = pbones[i].parent;
			}

			for(n=0;n<3;n++)
			{	iterRecBones->pos[i][n] = (*m_pbonetransform)[i][n][3];
			}
		}
	}
}
...
void CStudioModelRenderer::StudioMergeBones ( model_t *m_pSubModel )
{
	...
	recboneiter iterRecBones = gHUD.m_BonePos.FindBoneRecordWeapon(m_pCurrentEntity->index);
	if(iterRecBones != NULL)
	{	// Rendered this frame
		iterRecBones->blRenderedFrame = true;
		// Recording bones
	...
	for (i = 0; i < m_pStudioHeader->numbones; i++) 
	{
		...
		// Pcjoe: Copy to record bone struct
		if(record)
		{	// Only need to copy the name / parent bone once
			if(first)
			{	strcpy( iterRecBones->szName[i], pbones[i].name );
...
int CStudioModelRenderer::StudioDrawModel( int flags )
{
	...
	if (m_pCurrentEntity->curstate.renderfx == kRenderFxDeadPlayer)
	{
		...
		// added by jason bodies on top of ground
		m_pCurrentEntity->origin.z += 4;
...
//	gEngfuncs.Con_DPrintf("Player %.2f %.2f %.2f\n", pplayer->velocity[0], pplayer->velocity[1], pplayer->velocity[2] );

	// HACK HACK: Pcjoe - Copy over velocity
	m_pCurrentEntity->curstate.velocity = pplayer->velocity;
...
/*	Removed by Pcjoe
	...
	// Pcjoe: Set model like any other entity
	m_pRenderModel = m_pCurrentEntity->model;
	if(m_pRenderModel == NULL)
		return 0;

	// Pcjoe: Render check
	g_RenderCheck[m_pCurrentEntity->index].blFlag = true;
	g_RenderCheck[m_pCurrentEntity->index].blRendered = true;
	...
	if (pplayer->gaitsequence)
	{
		...
		// Added by Pcjoe - No gait on fly anim
		if(m_pCurrentEntity->curstate.movetype != MOVETYPE_PLAYERFLY)
	...
	/*	// Full lighting
		lighting.plightvec = vec3_t(0,0,0);
		lighting.ambientlight = 1;
		lighting.shadelight = 0;
		lighting.color = vec3_t(140,140,140);*/
		// model and frame independant
		IEngineStudio.StudioSetupLighting (&lighting);
...
void CStudioModelRenderer::StudioRenderFinal_Hardware( void )
{
	...
	{
		// do model replacement textures here
		std::string strModel = m_pCurrentEntity->model->name;
		if( !m_pCurrentEntity->syncbase )
		{
		
		
			strModel.erase( strModel.length( ) - 4, strModel.length( ) );
			
			int iPos = 0;
			for( int iCount = 0; iCount < strModel.length( ); iCount++ )
			{
				if( strModel[iCount] == '/' )
				{
					iPos = iCount + 1;
		...
		// added by jason
		if( gTexture.IsOpenGL( ) && !( m_pStudioHeader->flags & TEXTURE_REPLACE ) )
		{
			int iTex = 0;
			glGetIntegerv( GL_TEXTURE_BINDING_2D, &iTex );
			// replace textures here
			for( i = 0; i < m_pStudioHeader->numtextures; i++ )
			{

				if( gTexture.IsOpenGL( ) && !( m_pStudioHeader->flags & TEXTURE_REPLACE ) )
				{
					char szTmp[32];
					sprintf( szTmp, "%s%i\0", strModel.c_str(), i );
				
					//if( m_pStudioHeader->numtextures != 1 )
					{
						gTexture.OverwriteTexture( iTex-(i), szTmp, 0, true );
					}
				/*	else
					{
						gTexture.OverwriteTexture( iTex-1, szTmp, 0, true );
					}*/
				//	gEngfuncs.Con_Printf( "%i\n", iTex );
				}
			}
	
			m_pStudioHeader->flags += TEXTURE_REPLACE;
		}
...
void CStudioModelRenderer::StudioRenderFinal(void)
{
	...
	{
		// added by jason
		if( !g_Shadow.blRenderd )
		{

			g_Shadow.RenderShadow( );
			g_Shadow.blRenderd = true;
