...
int WeaponsResource :: HasAmmo( WEAPON *p )
{
	...
	// Edited by Pcjoe, mechmod weapons never run out of ammo
	return TRUE;
...
int CHudAmmo::VidInit(void)
{
	...
	// Pcjoe: Weapon select sprites
	m_HUD_wpleft = gHUD.GetSpriteIndex( "wp_left" );
	m_HUD_wpcenter = gHUD.GetSpriteIndex( "wp_center" );
	m_HUD_wpright = gHUD.GetSpriteIndex( "wp_right" );
	// Assume wpleft == wpright
	m_iweapedgeWidth = gHUD.GetSpriteRect(m_HUD_wpleft).right - gHUD.GetSpriteRect(m_HUD_wpleft).left;
	m_iweapedgeHeight = gHUD.GetSpriteRect(m_HUD_wpleft).bottom - gHUD.GetSpriteRect(m_HUD_wpleft).top;
	m_iweapcenterWidth = gHUD.GetSpriteRect(m_HUD_wpcenter).right - gHUD.GetSpriteRect(m_HUD_wpcenter).left;
	m_iweapcenterHeight = gHUD.GetSpriteRect(m_HUD_wpcenter).bottom - gHUD.GetSpriteRect(m_HUD_wpcenter).top;
...
//	int AmmoWidth;
	...
	// Removed by Pcjoe, we draw our ammo elsewhere
...
#define OFFSETBUCKET_X	15
#define OFFSETBUCKET_Y	10
#define BUCKET_WIDTH	128
#define BUCKET_HEIGHT	64
#define SLIDETIME		(float)0.25
...
*/
void CHudAmmo::SlideIn( void )
{	if(m_blSliding && !m_blSlideIn)
	{	// Invert time
		float flSlideTime = gHUD.m_flTime - m_flSlideTime;
		flSlideTime = SLIDETIME - flSlideTime;
		m_flSlideTime = gHUD.m_flTime - flSlideTime;
		m_blSlideIn = true;
	}
	else if(!m_blSliding)
	{	
...
* Pcjoe: Get Y Base
*/
int CHudAmmo::GetYBase( int iHeight )
{	int y=0;
	if(m_blSliding)
	{	y = -iHeight;
		float flTime = gHUD.m_flTime - m_flSlideTime;
		if(flTime > SLIDETIME)
			flTime = SLIDETIME;
		
		if(m_blSlideIn)
		{	y += iHeight * (1-(flTime/SLIDETIME));
		}
		else
		{	y += iHeight * (flTime/SLIDETIME);
		}
	}
	return y;
}
