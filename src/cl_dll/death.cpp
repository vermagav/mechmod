...
int CHudDeathNotice :: Draw( float flTime )
{
	...
	for ( int i = 0; i < MAX_DEATHNOTICES; i++ )
	{
		...
		if ( gViewPort && gViewPort->AllowedToPrintText() )
		{
			...
			// Pcjoe: Move death message down below radar (only when the radar is on / being drawn)
			if ( (gHUD.m_Radar.m_blRadar || gHUD.m_Radar.m_blSliding) && !((gHUD.m_iHideHUDDisplay & HIDEHUD_HEALTH) || gEngfuncs.IsSpectateOnly() || !(gHUD.m_iWeaponBits & (1<<(WEAPON_SUIT)))))
			{	y = gHUD.m_Radar.m_iyBottomRadar + 5 + (20 * i);
				if(y < (YRES(DEATHNOTICE_TOP) + 2 + (20 * i)))
					y = YRES(DEATHNOTICE_TOP) + 2 + (20 * i);
			}
