...
void	DrawSpriteQuad( char *pCTex, vec3_t vOrg );
float	ParseByComma( const char *pString, char *pSprite );
// end jason
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	...
	case 5004:		
		...
		break;
	// Added by jason Model event for sprite
	case 6999:
	//	gEngfuncs.Con_Printf( "Calling Event\n" );
		DrawSpriteQuad( (char *)event->options, entity->attachment[0] );
		break;
	default:
		break;
	}
...
float ParseByComma( const char *pString, char *pTex )
{
	if( !pTex )
	{
		return 0;
	...
	char rgcNum[256], rgcTexture[256];
	int i = 0;

	while( ( pString[i] != ',' && pString[i] != ' ' ) && !( i > 64 )  )
	{
		rgcTexture[i] = pString[i];
		i++;
	}
	rgcTexture[i] = '\0';
	...
	strcpy( pTex, rgcTexture );

	float flNum = atof( rgcNum );
...
{
	char rgcTexture[256];
	float flSize = ParseByComma( pCTex, rgcTexture );
	Texture_s *pTex = gTexture.LoadTexture( rgcTexture );
	...
	if(CL_IsThirdPerson())
	{
		AngleVectors( g_CameraAngles, NULL, vRight, vUp);	
	}
	else
	{	vec3_t vecViewAngles;
		gEngfuncs.GetViewAngles( (float *)vecViewAngles);
		AngleVectors( vecViewAngles, NULL, vRight, vUp);	
	}
	...
	if( pTex )
	{
		...
		{
			if( g_Glow.m_glActiveTexARB )
			{
				glGetIntegerv( GL_ACTIVE_TEXTURE_ARB, &iUnit );
				g_Glow.m_glActiveTexARB( GL_TEXTURE0_ARB );
			}
			
		}

		g_CGLTriAPI.RenderMode( kRenderTransAdd );//Add );

	
		g_CGLTriAPI.Texture2D( pTex, 0 );//iTime );
		...
		g_CGLTriAPI.Begin( TRI_QUADS );
			...
			vec3_t vPoint = vOrg + ( vRight * flSize ) - ( vUp * flSize );
			g_CGLTriAPI.Vertex3fv( vPoint );

			g_CGLTriAPI.TexCoord2f( 0, 1 );
			vPoint = vOrg - ( vRight * flSize ) - ( vUp * flSize );
			g_CGLTriAPI.Vertex3fv( vPoint );
