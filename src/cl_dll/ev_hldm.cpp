...
// Added by Pcjoe
void EV_RapidBulletFire( struct event_args_s *args  );
...
// Added by Pcjoe, global tracer system
CEffTracerBullet *g_TracerBullet;
	...
	for ( iShot = 1; iShot <= cShots; iShot++ )	
	{
		...
		if ( tr.fraction != 1.0 )
		{
			...
			case BULLET_PLAYER_MP5:		
				...
				// added by jason
				// check monster class
				if( gEngfuncs.GetEntityByIndex( idx )->curstate.playerclass == 4 )
				{
					GetManager()->AddSystem( new CEffExplosion( tr.endpos ) );
				}
				// end jason
...
{	int idx;
	...
	switch( args->iparam2 )
	{
	case 0: // VF1J's Rapid Fire and AF49's Rapid Fire
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/rapid_fire.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	break;
	case 1: // DS's Shield Gun / Laser Shot
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/laser_fire.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	break;
	case 2: // MK2's TopCannon Gun
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/quad_fire.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	break;
	case 3: // Freedom's Buster Shot
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/buster_shot.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	break;
	}
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MMRAPID, 2, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MMRAPID, 2, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	...
	if(!g_TracerBullet)
	{	GetManager()->AddSystem( g_TracerBullet = new CEffTracerBullet() );
	}

	// Get Angles
	vec3_t vecAngles, vecForward, vecRight, vecUp;
	...
	vec3_t vecDir;
	for(int i = 0; i < 3; i++)
		vecDir[i] = vecForward[i] + args->fparam1 * vecRight[ i ] + args->fparam2 * vecUp [ i ];

	// Emit particles
	// modifed by jason to use a type modifier
	g_TracerBullet->FillPart(vecSrc + vecDir * 20, vecDir * 3250, args->iparam1 );
}
