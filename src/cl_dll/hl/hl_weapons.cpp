...
// Pcjoe: MM weapons
CVf1j_RapidFire g_vf1j_rapidfire;
CMK2_RapidFire g_mk2_rapidfire;
CVf1j_AlphaBomb g_vf1j_alphabomb;
CFreedom_5Way	g_freedom_5way;
CFreedom_BusterSmall g_freedom_rapid;
// added by jason
CAf49_RapidFire g_af49_rapidfire;
CMK2_NukeMissile	g_mk2_nuke;
CFreedom_BusterBig	g_freedom_buster;
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	{	// Pcjoe: Reload MM weapon
		if(MMWeapon() && ((CMMWeapon*)this)->RapidFireWeapon())
	...
	// Added by Pcjoe, disable fire
	bool blCanFire = true;
#ifdef CLIENT_DLL
	if(gEngfuncs.GetLocalPlayer()->curstate.fuser4 > 0 || gEngfuncs.GetLocalPlayer()->curstate.vuser1[0])
	{	blCanFire = false;
	}
#endif
...
//	if ((m_pPlayer->pev->button & IN_ATTACK2) && (m_flNextSecondaryAttack <= 0.0))
	if ((m_pPlayer->pev->button & IN_ATTACK2) && blCanFire && (m_flNextSecondaryAttack <= 0.0))
	{
...
//	else if ((m_pPlayer->pev->button & IN_ATTACK) && (m_flNextPrimaryAttack <= 0.0))
	else if ((m_pPlayer->pev->button & IN_ATTACK) && blCanFire && (m_flNextPrimaryAttack <= 0.0))
	{
	...
	else if ( !(m_pPlayer->pev->button & (IN_ATTACK|IN_ATTACK2) ) )
	{
		...
		if ( m_iClip == 0 && !(iFlags() & ITEM_FLAG_NOAUTORELOAD) && m_flNextPrimaryAttack < 0.0 && !m_fInReload )
		{	Reload();
...
void HUD_InitClientWeapons( void )
{
	...
	// Pcjoe: MM weapons
	HUD_PrepEntity( &g_vf1j_rapidfire, &player );
	HUD_PrepEntity( &g_mk2_rapidfire, &player );
	HUD_PrepEntity( &g_vf1j_alphabomb, &player );
	HUD_PrepEntity( &g_freedom_5way, &player );
	HUD_PrepEntity( &g_freedom_rapid, &player );
	//added by jason
	HUD_PrepEntity( &g_af49_rapidfire, &player );
	HUD_PrepEntity( &g_mk2_nuke, &player );
	HUD_PrepEntity( &g_freedom_buster, &player );
}
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	{	// Pcjoe: MM weapons
		case WEAPON_VF1J_RAPID:
			pWeapon = &g_vf1j_rapidfire;
			break;
		case WEAPON_MK2_RAPIDFIRE:
			pWeapon = &g_mk2_rapidfire;
			break;
		case WEAPON_VF1J_ALPHA:
			pWeapon = &g_vf1j_alphabomb;
			break;
		case WEAPON_FREEDOM_5WAY:
			pWeapon = &g_freedom_5way;
			break;
		case WEAPON_FREEDOM_RAPID:
			pWeapon = &g_freedom_rapid;
			break;
		// added by jason
		case WEAPON_AF49_RAPIDFIRE:
			pWeapon = &g_af49_rapidfire;
			break;
		case WEAPON_MK2_NUKE:
			pWeapon = &g_mk2_nuke;
			break;
		case WEAPON_FREEDOM_BIG:
			pWeapon = &g_freedom_buster;
			break;
	}
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
		...
		pCurrent->pev->fuser2			= pfrom->fuser2;
		pCurrent->m_chargeReady			= pfrom->iuser1;
		pCurrent->m_fInAttack			= pfrom->iuser2;
		pCurrent->m_fireState			= pfrom->iuser3;

		// Added by Pcjoe, update charge bar information
		if(i == from->client.m_iId)
		{	g_flCurrentCharge = pCurrent->pev->fuser1;
			g_flMinCharge = pCurrent->pev->fuser2;
		}
