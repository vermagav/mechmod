...
#define CLASSMENU_TOPLEFT_BUTTON_X		XRES(12)
#define CLASSMENU_TOPLEFT_BUTTON_Y		YRES(100)
#define CLASSMENU_BUTTON_SIZE_X			XRES(116)
#define CLASSMENU_BUTTON_SIZE_Y			YRES(116)
//#define CLASSMENU_BUTTON_SPACER_Y		YRES(8)
#define CLASSMENU_BUTTON_SPACER_X		XRES(7)
...
/*	m_pScrollPanel->setScrollBarAutoVisible(false,true);
	...
	{
		char sz[256]; 
		int iXPos = CLASSMENU_TOPLEFT_BUTTON_X + ( (CLASSMENU_BUTTON_SIZE_X + CLASSMENU_BUTTON_SPACER_X) * ( i - 1 ) );

		ActionSignal *pASignal = new CMenuHandler_StringCommandClassSelect( sMMClassSelection[i], true );

		// Class button
		sprintf(sz, "\n" );//%s", CHudTextMessage::BufferedLocaliseTextString( "\n" ) );//sLocalisedClasses[i] ) );
		m_pButtons[i] = new ClassButton( i, "\n", iXPos, CLASSMENU_TOPLEFT_BUTTON_Y, CLASSMENU_BUTTON_SIZE_X, CLASSMENU_BUTTON_SIZE_Y, true);
		// RandomPC uses '0'
		if ( i >= 1 && i <= PC_RANDOM )
		{
			sprintf(sz,"%d",i);
	...
	/*	int textOffs = XRES(8);
		...
		sprintf(sz, "#Title_%s", sMMClassSelection[i]);
	...
	/*	if ( bShowClassGraphic )
		{
			for ( int team = 0; team < 2; team++ )
			{
				if ( team == 1 )
				{
					sprintf( sz, "%sred", sMMClassSelection[i] );
				}
				else
				{
					sprintf( sz, "%sblue", sMMClassSelection[i] );
				}
	...
	/*	gHUD.m_TextMessage.LocaliseTextString( "#Title_CurrentlyOnYourTeam", m_sPlayersOnTeamString, STRLENMAX_PLAYERSONTEAM );
		...
		sprintf(sz, "classes/short_%s.txt", sMMClassSelection[i]);
...
void CClassMenuPanel::Update()
{
	...
	if (!g_iTeamNumber && !(g_iUser1 || g_iUser2))
		return;

	gHUD.m_blHideHud = true;
...
void CClassMenuPanel::Open( void )
{
	...
	// moved here hope it helps Jason
