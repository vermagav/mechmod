...
//MECH MOD CODER_RICK
//int MechKilled = 0;
/*char player_A00[16]  = "A00";
char player_A01[16]  = "A01";
char player_A02[16]  = "A02";
char player_A03[16]  = "A03"; 
char player_A04[16]  = "A04";
char player_A05[16]  = "A05"; 
char player_A06[16]  = "A06";
char player_A07[16]  = "A07"; 
char player_A08[16]  = "A08";
char player_A09[16]  = "A09"; 
char player_A10[16]  = "A10";*/
//MECH MOD END CODER_RICK
...
SBColumnInfo g_ColumnInfo[NUM_COLUMNS] =
{
	...
	{"Level",		56,			Label::a_east},		// class
	{"#SCORE",		40,			Label::a_east},
	{"#DEATHS",		46,			Label::a_east},
	{"PING",	    46,			Label::a_east},
...
sprintf( sz2, CHudTextMessage::BufferedLocaliseTextString( team_info->name ) );
			...
			else
			{
				...
				case COLUMN_TRACKER:
					break;
			/*	case COLUMN_LEVEL:
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags )<5)
					{
					 sprintf(sz, "%s ", player_A00);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 5 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 10)
					{
					 sprintf(sz, "%s ", player_A01);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 10 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 15)
					{
					 sprintf(sz, "%s ", player_A02);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 15 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 20)
					{
					 sprintf(sz, "%s ", player_A03);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 20 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 25)
					{
					 sprintf(sz, "%s ", player_A04);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 25 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 30)
					{
					 sprintf(sz, "%s ", player_A05);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 30 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 35)
					{
					 sprintf(sz, "%s ", player_A06);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 35 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 40)
					{
					 sprintf(sz, "%s ", player_A07);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 40 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 45)
					{
					 sprintf(sz, "%s ", player_A08);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 45 && g_PlayerExtraInfo[ m_iSortedRows[row] ].frags < 50)
					{
					 sprintf(sz, "%s ", player_A09);
					}
					if ((g_PlayerExtraInfo[ m_iSortedRows[row] ].frags ) > 50)
					{
					 sprintf(sz, "%s ", player_A10);
					}
					break;*/
