...
try
{
	...
	while ( ( strlen ( token ) > 0 ) && ( m_iNumMenus < MAX_MENUS ) )
	{
		...
		while ( token[0] != '}' && ( strlen( token ) > 0 ) )
		{
			...
			{
				// See if it's a Class
				for (int i = 1; i < PC_LASTCLASS; i++)
				{
					if ( !strcmp(token, sMMClasses[i]) )
					{
						// Save it off
						iPlayerClass = i;
...
CCommandMenu *TeamFortressViewport::CreateDisguiseSubmenu( CommandButton *pButton, CCommandMenu *pParentMenu, const char *commandText, int iYOffset, int iXOffset )
{
	...
	for ( int i = PC_VF1J; i < PC_LASTCLASS; i++ )
	{
...
CommandButton *TeamFortressViewport::CreateCustomButton( char *pButtonText, char *pButtonName, int iYOffset )
{
	...
	else if ( !strcmp( pButtonName, "!CHANGECLASS" ) )
	{
		...
		for (int i = PC_VF1J; i <= PC_RANDOM; i++ )
		{
			...
			sprintf(sz, "%s", sMMClassSelection[i]);
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	else if ( iTextToShow == SHOW_CLASSDESC )
	{
		...
		{
		case PC_VF1J:		cText = CHudTextMessage::BufferedLocaliseTextString( "#Help_vf1j" ); 
							CHudTextMessage::LocaliseTextString( "#Title_vf1j", cTitle, MAX_TITLE_LENGTH ); break;
		case PC_AF49:		cText = CHudTextMessage::BufferedLocaliseTextString( "#Help_af49" );
							CHudTextMessage::LocaliseTextString( "#Title_af49", cTitle, MAX_TITLE_LENGTH ); break;
		case PC_DEATHSCYTHE:	cText = CHudTextMessage::BufferedLocaliseTextString( "#Help_deathscythe" );
							CHudTextMessage::LocaliseTextString( "#Title_deathscythe", cTitle, MAX_TITLE_LENGTH ); break;
		case PC_MK2:			cText = CHudTextMessage::BufferedLocaliseTextString( "#Help_mk2" );
							CHudTextMessage::LocaliseTextString( "#Title_mk2", cTitle, MAX_TITLE_LENGTH ); break;
		case PC_FREEDOM:	cText = CHudTextMessage::BufferedLocaliseTextString( "#Help_freedom" );
							CHudTextMessage::LocaliseTextString( "#Title_freedom", cTitle, MAX_TITLE_LENGTH ); break;
		default:
			return NULL;
		}
		...
		{
			sprintf(sz, "classes/long_%s.txt", sMMClassSelection[ g_iPlayerClass ]);
		}
...
{
	// Create the panel
	m_pTeamMenu = new CTeamMenuPanel( 0, false, 0, 0, ScreenWidth, ScreenHeight);
...
CMenuPanel* TeamFortressViewport::ShowClassMenu()
{
	...
	// Pcjoe: Don't open class menu if we're in teamplay and we dont have a team
	/*if(gHUD.m_Teamplay && !g_iTeamNumber)
		return NULL;*/
...
void TeamFortressViewport::CreateClassMenu()
{
	// Create the panel
	// modifed by jason
	m_pClassMenu = new CClassMenuPanel(0, false, 0, 0, ScreenWidth, ScreenHeight);
