...
#define TEAMMENU_BUTTON_SIZE_X			XRES(97)
#define TEAMMENU_BUTTON_SIZE_Y			YRES(48)
...
/*	int r, g, b, a;
	...
	pLabel->setText(gHUD.m_TextMessage.BufferedLocaliseTextString("Choose your side:"));
...
/*	for (int i = 1; i <= 5; i++)
	...
	m_pButtons[0] = new CommandButton( "", XRES( 145 ), YRES( 373 ), TEAMMENU_BUTTON_SIZE_X, TEAMMENU_BUTTON_SIZE_Y, true );
	m_pButtons[0]->setParent( this );
	m_pButtons[0]->setContentAlignment( vgui::Label::a_west );
	m_pButtons[0]->setVisible( true );
	m_pButtons[0]->addActionSignal( new CMenuHandler_StringCommandWatch( "jointeam 1", true ) );

	m_pButtons[1] = new CommandButton( "", XRES( 145 ) + 2 * TEAMMENU_BUTTON_SIZE_X + XRES( 54 ), YRES( 373 ), TEAMMENU_BUTTON_SIZE_X, TEAMMENU_BUTTON_SIZE_Y, true );
	m_pButtons[1]->setParent( this );
	m_pButtons[1]->setContentAlignment( vgui::Label::a_west );
	m_pButtons[1]->setVisible( true );
	m_pButtons[1]->addActionSignal( new CMenuHandler_StringCommandWatch( "jointeam 2", true ) );
...
/*	m_pCancelButton = new CommandButton( CHudTextMessage::BufferedLocaliseTextString( "#Menu_Cancel" ), TEAMMENU_TOPLEFT_BUTTON_X, 0, TEAMMENU_BUTTON_SIZE_X, TEAMMENU_BUTTON_SIZE_Y);
	...
	m_pSpectateButton = new SpectateButton( "", XRES( 145 ) + TEAMMENU_BUTTON_SIZE_X + XRES( 27 ), YRES( 373 ), TEAMMENU_BUTTON_SIZE_X, TEAMMENU_BUTTON_SIZE_Y, true);
...
/*	int	 iYPos = TEAMMENU_TOPLEFT_BUTTON_Y;
	...
	// Spectate button
	// Removed by Pcjoe, show spectator
