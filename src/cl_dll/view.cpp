...
// added by Pcjoe
extern cvar_t	*cam_xoffset;
extern cvar_t	*cam_yoffset;
...
{	// Added by Pcjoe, no bobbing in mechmod
	return 0;
...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	if( CL_IsThirdPerson() )
	{
		vec3_t ofs;
		vec3_t vecOriginal;
		vec3_t vecDir, vecDir2;
		vec3_t vecOrg;

		VectorCopy( pparams->vieworg, vecOriginal);
		...
		// added by jason
		VectorCopy( camAngles, g_vecCamAngles );
		...
		for ( i = 0; i < 3; i++ )
		{
			...
			// Added by Pcjoe, add offsets
			pparams->vieworg[ i ] += cam_xoffset->value * camRight[ i ];
			pparams->vieworg[ i ] += cam_yoffset->value * camUp[ i ];
		}

		// Pcjoe: Handle clipping
		pmtrace_t trace;
		for(i=0;i<3;i++)
			vecDir[i] = pparams->vieworg[i]-vecOriginal[i];
		VectorNormalize(vecDir);
		for(i=0;i<3;i++)
			vecDir2[i] = vecDir[i] * 16;
		for(i=0;i<3;i++)
			vecOrg[i] = pparams->vieworg[i] + vecDir2[i];
		...
		//	gEngfuncs.pEventAPI->EV_SetSolidPlayers ( pparams->playernum );	
			gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );
			gEngfuncs.pEventAPI->EV_PlayerTrace( vecOriginal,vecOrg, PM_WORLD_ONLY, -1, &trace );
		...
		// Reset view origin
		for( i = 0; i < 3; i++ )
			pparams->vieworg[i] = trace.endpos[i] - vecDir2[i];


		// added by jason sky checking
		while( gEngfuncs.PM_PointContents( pparams->vieworg, NULL ) == CONTENTS_SKY )
		{
			vec3_t vNorm = vecOriginal - vecOrg;

			vNorm = vNorm.Normalize( );

			for( i = 0; i < 3; i++ )
				pparams->vieworg[i] += vNorm[i];
		}

		// added by jason for celing clipping
		pmtrace_t trace1;
		vec3_t vTmp = pparams->vieworg;
		gEngfuncs.pEventAPI->EV_PlayerTrace( vTmp -vec3_t( 0, 0, 8 ), vTmp + vec3_t( 0, 0, 4096 ), PM_WORLD_ONLY, -1, &trace1 );

		if( trace.endpos.z == trace1.endpos.z )
		{
...
//	v_cl_angles = pparams->cl_viewangles;	// keep old user mouse angles !
	...
	// added by jason
	g_vViewOrg = pparams->vieworg;
	g_vPlayerAngles = pparams->viewangles;
...
void DLLEXPORT V_CalcRefdef( struct ref_params_s *pparams )
{
	...
	for( int i = 1; i < 32; i++ )
	{
		...
		if( pEnt->player )
		{
			if( !pEnt->model )
				continue;
			if( !stricmp( pEnt->model->name, "models/null.mdl" ) )
				continue;
			if( pEnt->curstate.effects & EF_NODRAW )
				continue;
			if( g_PlayerExtraInfo[i].teamnumber != 0  )
				g_Shadow.SetShadowOrg( pEnt->origin );
		}
	}
