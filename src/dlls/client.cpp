...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq(pcmd, "spectate" ) )
	{
		...
		if ( allow_spectators.value && g_pGameRules->IsTeamplay() || g_pGameRules->IsSeige( ) ) // modified by jason
...
/*	else if ( FStrEq( pcmd, "spectate" ) && (pev->flags & FL_PROXY) )	// added for proxy support
	...
	else if ( FStrEq( pcmd, "spec_mode" ) )
	{
	   ...
	   pPlayer->m_iPrevTeam = 0;	// added by jason
	}
	// Pcjoe: Toggle through fly
	else if (FStrEq(pcmd, "flynext") )
	{	GetClassPtr((CBasePlayer *)pev)->NextFlyMode();
	}
	else if (FStrEq(pcmd, "flyprev") )
	{	GetClassPtr((CBasePlayer *)pev)->PrevFlyMode();
	}
	// Pcjoe: Power to energy/armor conversion
	else if (FStrEq(pcmd, "powerconvert" ))
	{	GetClassPtr((CBasePlayer *)pev)->PowerToEnergy();
	}
	// Added by Rick
    else if (FStrEq(pcmd, "teamswitch" ))
	{	if(g_pGameRules->IsTeamplay() || g_pGameRules->IsSeige( ) ) // modifed by Jason
			GetClassPtr((CBasePlayer *)pev)->ShowVGUI( MENU_TEAM ); // Rick: MENU_TEAM hopefully switches da team
	}
	// Pcjoe: Change Class
	else if (FStrEq( pcmd, "changeclass" ))
	{	GetClassPtr((CBasePlayer *)pev)->ShowVGUI( MENU_CLASS );
	}
	// Added by Pcjoe/grOOvy
	else if (FStrEq(pcmd, "vf1j" ) )
	{	GetClassPtr((CBasePlayer *)pev)->ChangeNumericalClass(PC_VF1J);
	}
	else if (FStrEq(pcmd, "af49" ) )
	{	GetClassPtr((CBasePlayer *)pev)->ChangeNumericalClass(PC_AF49);
	}
	else if (FStrEq(pcmd, "deathscythe" ) )
	{	GetClassPtr((CBasePlayer *)pev)->ChangeNumericalClass(PC_DEATHSCYTHE);
	}
	else if (FStrEq(pcmd, "mk2" ) )
	{	GetClassPtr((CBasePlayer *)pev)->ChangeNumericalClass(PC_MK2);
	}
	else if (FStrEq(pcmd, "freedom" ) )
	{	GetClassPtr((CBasePlayer *)pev)->ChangeNumericalClass(PC_FREEDOM);
	}
	else if (FStrEq(pcmd, "randompc" ) )
	{	GetClassPtr((CBasePlayer *)pev)->ChangeNumericalClass(RANDOM_LONG(PC_VF1J, PC_RANDOM-1));
	}
	// End of add
...
//	gpGlobals->teamplay = teamplay.value;
	if(gamemode.value == GAMEMODE_TEAMPLAY || gamemode.value == GAMEMODE_SEIGE ) // modifed by jason
...
//	PRECACHE_MODEL("models/player.mdl");
	...
	// Added by Pcjoe
	PRECACHE_SOUND("player/boost_steady.wav");
	PRECACHE_SOUND("player/boost_turbo.wav");
	PRECACHE_SOUND("player/boost_doublejump.wav");
	PRECACHE_SOUND("player/boost_stop.wav");
	PRECACHE_SOUND("player/stomp1.wav");
	PRECACHE_SOUND("player/stomp2.wav");
	PRECACHE_SOUND("player/stomp3.wav");
	PRECACHE_SOUND("player/mech_respawn.wav");
	PRECACHE_SOUND("player/hit_ground.wav");
	PRECACHE_SOUND("player/death_explosion.wav");
	PRECACHE_SOUND("player/missle_fire.wav");

	// Precache player models
	PRECACHE_MODEL("models/player/vf1j/vf1j.mdl");
	PRECACHE_MODEL("models/player/silveraxe/silveraxe.mdl");
	PRECACHE_MODEL("models/player/deathscythe/deathscythe.mdl");
	PRECACHE_MODEL("models/player/freedom/freedom.mdl");
	PRECACHE_MODEL("models/player/monster/monster.mdl");

	PRECACHE_MODEL("models/null.mdl");

	UTIL_PrecacheOther("boostfx");
	UTIL_PrecacheOther("attachent");
	UTIL_PrecacheOther("missle");
	UTIL_PrecacheOther("alphabomb");
	UTIL_PrecacheOther("wastedmissle");
	UTIL_PrecacheOther("genericmissle");
	UTIL_PrecacheOther("freedomblast");
...
void SetupVisibility( edict_t *pViewEntity, edict_t *pClient, unsigned char **pvs, unsigned char **pas )
{
	...
	// Pcjoe: Robin Walker - Tracking Spectators use the visibility of their target
...
//		state->usehull      = ( ent->v.flags & FL_DUCKING ) ? 1 : 0;
		...
		state->vuser1[0]	= ent->v.vuser1[0];
		state->vuser1[1]	= ent->v.vuser1[1];
	}
...
//						item->fuser1					= max( gun->pev->fuser1, -0.001 );
						item->fuser1					= gun->pev->fuser1;
						item->fuser2					= gun->pev->fuser2;
...
/*			cd->fuser2			= pl->m_flNextAmmoBurn;
			...
			if ( pl->m_pActiveItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					// Added by Pcjoe: Temp fix... might work
					cd->vuser2.z	= gun->m_iClip;
...
void CmdStart( const edict_t *player, const struct usercmd_s *cmd, unsigned int random_seed )
{
	...
	// added by jason
	UTIL_SetSize( pl->pev, VEC_HULLS_MIN, VEC_HULLS_MAX );
}
...
int GetHullBounds( int hullnumber, float *mins, float *maxs )
{
	...
	switch ( hullnumber )
	{
		// Added by Pcjoe, if trying to use duck, use normal player hull
	//case 1:
	//case 3:
