...
/*
Rewritten to throw custom mech heads
@Author: Gav 'grOOvy' Verma
@Last Modified: 14th November 2004
*/
void CGib :: SpawnHeadGib( entvars_t *pevVictim, int iCheckClass )
{
	CGib *pGib = GetClassPtr( (CGib *)NULL );
	...
	// VF1J
	case 1:	
		pGib->Spawn( "models/gibs/vf1j/head.mdl" ); // throw one head
		pGib->pev->body = 0;
		break;
	// AF49
	case 2:
		pGib->Spawn( "models/gibs/af49/head.mdl" ); // throw one head
		pGib->pev->body = 0;
		break;
	// DS
	case 3:
		pGib->Spawn( "models/gibs/ds/head.mdl" ); // throw one head
		pGib->pev->body = 0;
		break;
	// MK2
	case 4:
		pGib->Spawn( "models/gibs/mk2/head.mdl" ); // throw one head
		pGib->pev->body = 0;
		break;
	// FREEDOM
	case 5:
		pGib->Spawn( "models/gibs/freedom/head.mdl" ); // throw one head
		pGib->pev->body = 0;
		break;    
	default: break; // none of the above, do nothing.
	}
...
/*
Rewritten to throw custom mech parts
@Author: Gav 'grOOvy' Verma
@Last Modified: 14th November 2004
*/
void CGib :: SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human, int iCheckClass )
{
	int cSplat;

	for ( cSplat = 1 ; cSplat <= cGibs ; cSplat++ )
		// It is important that cSplat starts at 1 and not 
		// to match the file names of the gibs. :|
	{
		CGib *pGib = GetClassPtr( (CGib *)NULL );
		...
		std::string strPartName = "models/gibs/";
		char strTemp[10];
		itoa (cSplat, strTemp, 10);

		switch (iCheckClass)
		{
		case 1:
			strPartName.append("vf1j/part");
			strPartName.append(strTemp);
			strPartName.append(".mdl");
			pGib->Spawn( strPartName.c_str() );
            //pGib->pev->body = RANDOM_LONG(1,HUMAN_GIB_COUNT-1);
			break;
		case 2:
			strPartName.append("af49/part");
			strPartName.append(strTemp);
			strPartName.append(".mdl");
			pGib->Spawn( strPartName.c_str() );
            //pGib->pev->body = RANDOM_LONG(1,HUMAN_GIB_COUNT-1);
		...
		case 4:
			...
			break;
		case 5:
			strPartName.append("freedom/part");
			strPartName.append(strTemp);
			strPartName.append(".mdl");
			pGib->Spawn( strPartName.c_str() );
            //pGib->pev->body = RANDOM_LONG(1,HUMAN_GIB_COUNT-1);
...
/*
Rewritten to give custom Mecha part gibs
@Author: Gav 'grOOvy' Verma
@Last Modified: 14th November 2004
*/
...
void CBaseMonster :: GibMonster( void )
{
	...
	// Emit mech explosion sound
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "player/death_explosion.wav", 1, ATTN_NORM);
	
	// Variable used to store clas
	int iCheckClass = pev->playerclass;

	if ( iCheckClass != 0 ) // Make sure class isn't PC_NONE (no class)
	{
        CGib::SpawnHeadGib ( pev, iCheckClass );
		CGib::SpawnRandomGibs( pev, 5, 1, iCheckClass );
		gibbed = TRUE;
	}
...
void CGib :: Spawn( const char *szGibModel )
{
	...
	m_material = matNone;
	// changed to 0 (was 5) for no blood decals by grOOvy: 14/11/04
