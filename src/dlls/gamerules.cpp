...
edict_t *CGameRules :: GetPlayerSpawnSpot( CBasePlayer *pPlayer )
{
	...
	if( !pentSpawnSpot )
	{
		pentSpawnSpot = UTIL_EntityByIndex( 0 )->edict( );
	}
...
CGameRules *InstallGameRules( void )
{
	...
	char *pName = ( char * )STRING( gpGlobals->mapname );
	if( !memcmp( pName, "se_", 3 ) )
	{
		// if we have even one objective make it seige
		gamemode.value = GAMEMODE_SEIGE;
	}
	// end check

	// Teamplay
	if( gamemode.value == GAMEMODE_TEAMPLAY )
	{	return new CHalfLifeTeamplay;
	}
	// Added by Jason 1-28-04
	else if( gamemode.value == GAMEMODE_SEIGE )
	{
		return new CMechModSeige;
	}
	// End Jason
	// Deathmatch (default)
	else
    {	gamemode.value = GAMEMODE_DEATHMATCH;
