...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
	...
	// added by grOOvy
	extern int gmsgScore;
	// end grOOvy
	// Added by Pcjoe, create class
	pl->m_pClass = new CBasePlayerClass;
	pl->m_pClass->SetOwner( pl );	// Set owner
	pl->m_iPlayerClass = 0;			// Set player class
	pl->pev->playerclass = 0;
	// added by grOOvy
	pl->setScore( 0 ); // Set's player score to 0 on map change
	// end grOOvy
...
/*	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( plr )
		{
			...
			// Pcjoe: Robin Walker - Send their spectator state
	...
	// Pcjoe: Send class select
	if(gamemode.value == GAMEMODE_DEATHMATCH)
	{	pl->ShowVGUI (MENU_CLASS);
	}
}
...
void CHalfLifeMultiplay :: ClientDisconnected( edict_t *pClient )
{
	if ( pClient )
	{
		...
		if ( pPlayer )
		{
			...
			// Added by Pcjoe, Robin Walker's Spectator
...
/*	addDefault = TRUE;
	...
	// Added by Pcjoe, class check
	if(pPlayer->m_iPlayerClass < PC_VF1J || pPlayer->m_iPlayerClass >= PC_RANDOM)
	{	pPlayer->m_iPlayerClass = 0;
		pPlayer->pev->playerclass = 0;
	}

	if (!pPlayer->m_iPlayerClass)
    {
		...
		// Added by Pcjoe 
		pPlayer->m_flSlowDown = 0.0; // Stop movement
    }
    else
    {	// Make sure the player can see the HUD again
		...
		// Added by Pcjoe
		// Only set class if we are changing classes
		if(pPlayer->m_iPlayerClass != pPlayer->m_pClass->ClassID())
		{	pPlayer->ChangeClass();
		}

		// Set Owner
		pPlayer->m_pClass->SetOwner( pPlayer );
		// Give Weapons
		pPlayer->m_pClass->GiveWeapons();
		// Give Saber Attributes
		pPlayer->m_pClass->AddSaberStyles();
		// Set Health
		pPlayer->m_pClass->SetHealth();
		//Rick
		//Set Armor
		pPlayer->m_pClass->SetArmor();
		//End Rick
		// Send max health/armor to client
		pPlayer->m_pClass->SendMaxHealth();
		// Set Model
		pPlayer->m_pClass->SetModel();
		// Update Client Info
		pPlayer->UpdateClientData();
		pPlayer->UpdateScoreBoard();
		
		// Force third person on spawn
		CLIENT_COMMAND (pPlayer->edict(),  "thirdperson\n");

		// Spawn Sound
		EMIT_SOUND(ENT(pPlayer->pev), CHAN_ITEM, "player/mech_respawn.wav", 1, ATTN_NORM);
	}
}
...
edict_t *CHalfLifeMultiplay::GetPlayerSpawnSpot( CBasePlayer *pPlayer )
{
	...
	// added by jason for map check
	if( !stricmp( STRING( gpGlobals->mapname ), "mech_orb" ) )
	{
		// SETUP HARD CODED SPAWNS for orb
		UTIL_SetOrigin( pPlayer->pev, vec3_t( 0, 0, 80 ) );
	}
