...
//added by jason 1-9-04
#include "part_ent.h"
#include "../cl_dll/hull_def.h"
// added 1-14-04
...
// Pcjoe: Fuel cost
#define FUEL_FLY_NORMAL		35
#define FUEL_FLY_FAST		70
#define FUEL_BOOST			150
#define FUEL_ADD_NEG		35
#define FUEL_ADD_GROUND		125
#define FUEL_ADD_AIR		60

// Pcjoe: Power conversions/cost
#define POWER_CONVERT_TAKE	0.33
#define POWER_CONVERT_ADD_F	1.0
#define POWER_CONVERT_ADD_A	0.33
#define POWER_ADD			MAX_POWER/120	// Fills max power in 120 seconds
#define POWER_CONVERT_DELAY	6

// Pcjoe: Negitive fuel time
#define NEGITIVE_FUEL_TIME	2.5
...
//	gmsgSelAmmo = REG_USER_MSG("SelAmmo", sizeof(SelAmmo));
	...
	gmsgBoost = REG_USER_MSG("Boostfx", 4);
	gmsgSaber = REG_USER_MSG("Saberfx", 3);
	gmsgPower = REG_USER_MSG("Power", 2);
	gmsgFlyMode = REG_USER_MSG("Flymode", 1);
	gmsgLockon = REG_USER_MSG("Lockon", 2);
	gmsgMaxHealth = REG_USER_MSG("MaxHealth",2);
	gmsgLockMdl = REG_USER_MSG("LockMdl",3);
	// added by jason 1-9-03
	gmsgPartMsg = REG_USER_MSG( "PartMsg", -1 );
	gmsgDecalMsg = REG_USER_MSG( "DecalMsg", 1 );
	gmsgEntTrail = REG_USER_MSG( "EntTrail", -1 );
	// end jason
	//added by grOOvy
	gmsgScore = REG_USER_MSG( "Score", 4 );
	//end grOOvy
...
{	if (IsAlive() && !m_fFlying && !m_fNegFuel && !m_fImpact)
	{	
		m_fFlying = TRUE;
		...
		// Added by Pcjoe
		EMIT_SOUND(ENT(pev), CHAN_BODY, "player/boost_steady.wav", 1, ATTN_NORM); // Emit sound
		// Enable Boosters
		if(m_pBoost)
		{	if(m_pBoost->m_fDoubleJump)
			{	m_pBoost->m_fDoubleJump = FALSE;
				m_pBoost->SetThink( NULL );
				m_pBoost->SendBoostMsg();
			}
			else
			{	m_pBoost->Remove();
				m_pBoost = NULL;
		...
		if(!m_pBoost)
			m_pBoost = CBoost::CreateBoost(this, FALSE);
		// Send info to client
		SendFlyMode();
	}
}
...
{	if (m_fFlying)
	{	m_fFlying = FALSE;
		...
		// added by Pcjoe
		EMIT_SOUND(ENT(pev), CHAN_BODY, "player/boost_stop.wav", 1, ATTN_NORM); // Emit sound
		// Disable Boosters
		if(m_pBoost)
		{	m_pBoost->Remove();
			m_pBoost = NULL;
...
{	// Set Class system
	...
	case PC_AF49:
		...
		break;
	case PC_DEATHSCYTHE:
		m_pClass = new CDeathscythe;
		break;
	case PC_MK2:
		m_pClass = new CMK2;
		break;
	case PC_FREEDOM:
		m_pClass = new CFreedom;
		break;
	}
}

void CBasePlayer::ChangeNumericalClass(int iClass)
{	// Don't allow change team if player isn't a part of a team
	// modifed by jason

	if( m_iPlayerClass && IsAlive( ) && m_flRespawnTime > gpGlobals->time )
	{
		return;
	}

	if( !m_flRespawnTime && IsAlive( ) )
	{
		m_iNextClass = iClass;
		m_flRespawnTime = gpGlobals->time + 5;
		return;
	}

	if(g_pGameRules->IsTeamplay() || g_pGameRules->IsSeige( ))
	{	if(!pev->team)
		{	return;
		}
	}
	...
	{	m_iPlayerClass = iClass;
		pev->playerclass = iClass;
	
		// Player is an observer, spawn player
		if(FBitSet(m_afPhysicsFlags, PFLAG_OBSERVER))
		{	m_afPhysicsFlags &= ~PFLAG_OBSERVER;
			StopObserver();
			Spawn();
		}
		// Inflict massive damage to the player 
		else
		{	entvars_t *pevWorld = VARS( INDEXENT(0) );
			TakeDamage( pevWorld, pevWorld, 999, DMG_NEVERGIB );
		}
	}
}

void CBasePlayer::UpdateScoreBoard( void )
{	MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		WRITE_BYTE( ENTINDEX(edict()) );
		WRITE_SHORT( pev->frags );
		WRITE_SHORT( m_iDeaths );
		WRITE_SHORT( m_iPlayerClass );
		WRITE_SHORT( g_pGameRules->GetTeamIndex( m_szTeamName ) + 1 );
	MESSAGE_END();
}

void CBasePlayer::LocalUpdateScoreBoard( void )
{	MESSAGE_BEGIN( MSG_ONE, gmsgScoreInfo, NULL, pev );
		WRITE_BYTE( ENTINDEX(edict()) );
		WRITE_SHORT( pev->frags );
		WRITE_SHORT( m_iDeaths );
		WRITE_SHORT( m_iPlayerClass );
...
*/
void CBasePlayer::DisableSpecialModes( void )
{	m_fImpact = FALSE;
	m_fNegFuel = FALSE;
	m_fDoubleJump = FALSE;
	m_flNegFuel = 0;
	m_flKilled = gpGlobals->time;	// Set killed time
	pev->fuser3 = 0;
	pev->iuser3 = 0;
	...
	{	m_pBoost->Remove();
		m_pBoost = NULL;
		EMIT_SOUND(ENT(pev), CHAN_BODY, "common/null.wav", 1, ATTN_NORM); // Emit sound
	}

	// Disabled attached model if present
	DisableAttachMdl();

	// Disable model lock (incase it's active)
	UTIL_LockModelAngles( (CBaseEntity*)this, FALSE );
...
*/
void CBasePlayer::SendFlyMode( void )
{	MESSAGE_BEGIN( MSG_ONE, gmsgFlyMode, NULL, pev );
		WRITE_BYTE( m_iFlyMode );
	MESSAGE_END();
}
...
* Next Fly Mode
*/
void CBasePlayer::NextFlyMode( void )
{	if(IsAlive() && !m_fNegFuel && !m_fImpact && !m_fFreedomBlast)
	{	m_iFlyMode++;
		if(m_iFlyMode > FLY_BOOST)
			m_iFlyMode = FLY_NONE;
	}
...
* Prev Fly Mode
*/
void CBasePlayer::PrevFlyMode( void )
{	if(IsAlive() && !m_fNegFuel && !m_fImpact && !m_fFreedomBlast)
	{	m_iFlyMode--;
		if(m_iFlyMode < FLY_NONE)
			m_iFlyMode = FLY_BOOST;
	}
...
* Toggle Flight
*/
void CBasePlayer::ToggleFlight( void )
{	// Enable Flight

	// ADDED BY JASON MK2 FLY CHECK
	if( pev->playerclass == PC_MK2 )
	{
		return;
	}

	if(!m_fFlying && m_iFlyMode != FLY_NONE)
	{	StartFly();
	}
	// Disable Flight
	else if(m_fFlying && m_iFlyMode == FLY_NONE)
	{	StopFly();
	}
	// Send fly mode info
	else
	...
	// Enable boost
	if(!m_fBoost && m_iFlyMode == FLY_BOOST)
	{	EnableBoost();
	}
	// Disable Boost
	else if(m_fBoost && m_iFlyMode != FLY_BOOST)
	{	DisableBoost();
	}
}
...
* Get Boost Direction
*/
int CBasePlayer::GetBoostDir( void )
{	if(pev->button & IN_FORWARD)
	...
	else if((pev->button & IN_MOVELEFT) || (pev->button & IN_LEFT))
	{	return BOOST_LEFT;
	}
	else if((pev->button & IN_MOVERIGHT) || (pev->button & IN_RIGHT))
	{	return BOOST_RIGHT;
	}
	else if(pev->button & IN_JUMP)
	{	return BOOST_UP;
	}
...
* Handle Boost
*/
void CBasePlayer::HandleBoost( void )
{	if(m_fBoost)
	{	pev->vuser1[0] = GetBoostDir();
	}
}
...
* Enable Boost
*/
void CBasePlayer::EnableBoost( void )
{	// Enable and set movetype
	m_fBoost = TRUE;
	pev->movetype = MOVETYPE_PLAYERBOOST;

	// Set Direction
	pev->vuser1[0] = BOOST_FRONT;
	pev->vuser1[0] = GetBoostDir();

	// Play boost sound
	EMIT_SOUND(ENT(pev), CHAN_BODY, "player/boost_turbo.wav", 1, ATTN_NORM); // Emit sound
}

/*
* Disable Boost
*/
void CBasePlayer::DisableBoost( void )
{	// Disable boost
	m_fBoost = 0;
	pev->vuser1[0] = BOOST_NONE;
...
*/
float CBasePlayer::GetFlightMultiplier( void )
{	if(m_fFlying && m_pClass)
	{	switch(m_iFlyMode)
		{	case FLY_NORMAL:
				return m_pClass->FlyNormal();
			case FLY_FAST:
				return m_pClass->FlyFast();
			case FLY_BOOST:
				return m_pClass->FlyBoost();
		}
	}
	return 1.0;
}
...
/*
* Pcjoe: Get Max Speed of player
*/
int CBasePlayer::GetMaxSpeed( void )
{	return m_pClass->MaxSpeed() * m_flSlowDown * GetFlightMultiplier();
}

/*
* Pcjoe: Can Use Fuel
*/
BOOL CBasePlayer::CanUseFuel( float flAmount )
{	flAmount *= m_pClass->FuelEfficiency();
	if(m_fNegFuel || (flAmount > pev->fuser3))
	{	return FALSE;
	}
	return TRUE;
}
...
* Pcjoe: Use Fuel
*/
BOOL CBasePlayer::UseFuel( float flAmount )
{	if(CanUseFuel(flAmount))
	{	pev->fuser3 -= flAmount * m_pClass->FuelEfficiency();
		return TRUE;
	}
	return FALSE;
}
...
* Pcjoe: Add Fuel
*/
void CBasePlayer::AddFuel( float flAmount )
{	// Reg fuel adding
	if(!m_fNegFuel)
	{	pev->fuser3 += flAmount;
		if(pev->fuser3 > MAX_FUEL)
			pev->fuser3 = MAX_FUEL;
	}
	// Negitive fuel adding
	else
	{	pev->fuser3 -= flAmount;
		if(pev->fuser3 < -MAX_FUEL)
			pev->fuser3 = -MAX_FUEL;
...
* Pcjoe: Use Power
*/
BOOL CBasePlayer::UsePower( float flAmount )
{	if(CanUsePower(flAmount))
	{	m_flPower -= flAmount;
		return TRUE;
	}
	return FALSE;
...
* Pcjoe: Add Power
*/
void CBasePlayer::AddPower( float flAmount )
{	m_flPower += flAmount;
	if(m_flPower > MAX_POWER)
	{	m_flPower = MAX_POWER;
	}
}
...
* Can Use Power
*/
BOOL CBasePlayer::CanUsePower( float flAmount )
{	if(flAmount > m_flPower)
	{	return FALSE;
	}
	return TRUE;
}
...
/*
* Pcjoe: Convert Power To Energy
*/
void CBasePlayer::PowerToEnergy( void )
{	if(IsAlive() && !m_fImpact && !m_fBoost && !m_fNegFuel && (m_flPowerConvert < gpGlobals->time))
	{	// Take away power
		if(UsePower(POWER_CONVERT_TAKE*MAX_POWER))
		{	// Add Delay
			m_flPowerConvert = gpGlobals->time + POWER_CONVERT_DELAY;
			// Refill fuel and add armor
			AddFuel(POWER_CONVERT_ADD_F*MAX_FUEL);
			// Add 1/3rd armor
			pev->armorvalue += m_pClass->MaxArmor() * POWER_CONVERT_ADD_A;
			if(pev->armorvalue > m_pClass->MaxArmor())
				pev->armorvalue = m_pClass->MaxArmor();
		}
	}
}

/*
* Pcjoe: Activate Negitive Fuel mode
*/
void CBasePlayer::StartNegFuel( float flTime )
{	// Set negfuel mode on
	m_flNegFuel = gpGlobals->time + flTime;
	m_fNegFuel = TRUE;

	// Set fuel to negitive
	pev->fuser3 = -pev->fuser3;
}

/*
* Pcjoe: Check Negitive Fuel mode
*/
void CBasePlayer::CheckNegFuel( void )
{	if(m_fNegFuel)
	{	if(m_flNegFuel < gpGlobals->time)
		{	StopNegFuel();
		}
	}
}
...
/*
* Pcjoe: Disable Negitive Fuel mode
*/
void CBasePlayer::StopNegFuel( void )
{	// Set negfuel mode off
	m_flNegFuel = 0;
	m_fNegFuel = FALSE;

	// Flip fuel again to set back to positive
	pev->fuser3 = -pev->fuser3;
}
...
* Pcjoe: Handle Fuel
*/
void CBasePlayer::HandleFuel( void )
{	// Check negitive fuel
	CheckNegFuel();
	// Fuel cost while flying
	if(m_fFlying)
	{	float flAmount = 0;
		
		// Get flying fuel cost
		switch(m_iFlyMode)
		{
		default:
		case FLY_NORMAL:
			flAmount = FUEL_FLY_NORMAL;
			break;
		case FLY_FAST:
			flAmount = FUEL_FLY_FAST;
			break;
		case FLY_BOOST:
			flAmount = FUEL_BOOST;
			break;
		...
		if(!UseFuel(flAmount * gpGlobals->frametime))
		{	// Stop flight, start negitive fuel mode (only if we're not firing a freedom blast)
			if(!m_fFreedomBlast)
			{	StopFly();
				StartNegFuel(NEGITIVE_FUEL_TIME);
			}
		}
	}
	// Refil negitive fuel
	else if(m_fNegFuel)
	{	AddFuel( FUEL_ADD_NEG * gpGlobals->frametime );
	}
	// Refill in air
	else if(!FBitSet( pev->flags, FL_ONGROUND ))
	{	AddFuel( FUEL_ADD_AIR * gpGlobals->frametime );
	}
	// Refill on ground
	else
	{	AddFuel( FUEL_ADD_GROUND * gpGlobals->frametime );
	}

	// Handle power as well
	AddPower( POWER_ADD * gpGlobals->frametime );
}
...
*/
void CBasePlayer::EnabledAttachMdl( char *pModel )
{	if(m_pAttachEnt)
	{	m_pAttachEnt->SetModel( pModel );
	}
	else
	{	m_pAttachEnt = CAttachEnt::CreateModel( this, pModel );
	}
}
...
*/
void CBasePlayer::DisableAttachMdl( void )
{	if(m_pAttachEnt)
	{	m_pAttachEnt->Disable();
	}
	m_pAttachEnt = NULL;
}
...
//=========================================================
// Score Module, by grOOvy --> Begin
//=========================================================


void CBasePlayer::incScore ( int x )
{ // function to increment score by x amount
	m_iPlayerScore += x;
}

void CBasePlayer::decScore ( int x )
{ // function to decrement score by x amount 
	if ( (m_iPlayerScore - x) < 0 )
		m_iPlayerScore = 0; // Making sure that the score doesn't go below zero

	else
		m_iPlayerScore -= x;
}

void CBasePlayer::setScore ( int x )
{	// used for manually setting the score
	m_iPlayerScore = x;
}

// added 17/04/2004

void CBasePlayer::setBlastScore ( int x )
{	// this determines how much score should be awarded depending on the weapon which was used
	switch(x)
	{	
	case 1: incScore(3); break; // Homing Missile(s)
	case 2: incScore(20); break; // VF-1J's Alphabomb
	case 3: incScore(30); break; // AF-49's Wasted Missile
	// add more here as we code in more attacks/weapons
...
void CBasePlayer :: DeathSound( void )
{
	...
	*/
	...
	// Pcjoe: Death Sound
	EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/death_explosion.wav", 1, ATTN_NORM);
}
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if ( pev->takedamage )
	{
		...
		// Edited by Pcjoe, spawn sparks instead of blood, mechs dont bleed
...
/*
* Pcjoe: Add damage power
*/
void CBasePlayer::AddDamagePower( entvars_t *pevInflictor, float flDamage, int bitsDamageType )
{	CBaseEntity *pAttacker = CBaseEntity::Instance(pevInflictor);
	
	// TODO: Give missles their own power restore damage ratios
/*	if(pAttacker->Classify() == CLASS_MISSLE)
	{
	}*/

	// Blast damamge
	if(bitsDamageType & DMG_BLAST)
	{	AddPower(flDamage * 0.75);
	}
	// FIXME: When saber damamge is implemented remove DMG_CLUB
	else if(bitsDamageType & DMG_CLUB)
	{	AddPower(flDamage * 2.25);
	}
	else if(bitsDamageType & DMG_BULLET)
	{	AddPower(flDamage * 1.75);
	}
	// Generic damage
	else
	{	AddPower(flDamage);
...
*/
//Rick
#define ARMOR_RATIO	 0.0	// Armor Takes 80% of the damage
#define ARMOR_BONUS  0.5	// Each Point of Armor is work 1/x points of health //Rick Was .5
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// added by grOOvy
	CBaseEntity *pInflict = CBaseEntity::Instance(pevInflictor);

	if ( pAttacker->IsPlayer() && pev!=pevAttacker )
	{	CBasePlayer *pPlrAttacker = (CBasePlayer *)pAttacker;

		if ( bitsDamage & DMG_BULLET )
			pPlrAttacker->incScore(1);
		if ( bitsDamageType & DMG_BLAST )
			pPlrAttacker->incScore( ((CBaseProjectile*)pInflict)->GetScoreIncrement() ); // modified 26/07/2004

		// Pcjoe: Add power to attacker
		pPlrAttacker->AddDamagePower(pevInflictor, flDamage, bitsDamageType);
	}
...
{
	// added by grOOvy: Awarding score to attacker for killing someone
	CBaseEntity *pAttacker = CBaseEntity::Instance(pevAttacker);
	
	if ( pAttacker && pAttacker->IsPlayer() && (pev != pevAttacker) )
        ((CBasePlayer *)pAttacker)->incScore(10); // Incrementing score
	else
		if	( pev == pevAttacker )
			decScore(10); // decrementing score
	// end grOOvy
...
//	SetAnimation( GetDeathActivity() );
	SetAnimation( ACT_DIESIMPLE );
...
//	pev->modelindex = g_ulModelIndexPlayer;    // don't use eyes
	...
	// disable bbox by jason
	g_SaberPhys.RemovePlayerBox( this );
...
* Play Animation
*/
void CBasePlayer::PlayAnimation( void )
{	// Manage anim list
	m_Anim.ManageAnimList();
	std::list<entanim_s>::iterator iterAnim = m_Anim.GetCurrentAnim();

	// Play Full Body Animation
	if(!FullBodyAnimation(iterAnim))
	{	// Play upper body + gait animations
		int iAnimDesired = UpperBodyAnimation(iterAnim);
		pev->gaitsequence = GaitAnimation(iterAnim);
...
*/
BOOL CBasePlayer::FullBodyAnimation( std::list<entanim_s>::iterator iterAnim )
{	int animDesired = 0;

	// Death animations already taken care of
	if(!IsAlive())
	{	return TRUE;
	}
	else if(m_fImpact)
	{	animDesired = LookupSequence( "land_impact" );
		m_IdealActivity = ACT_IMPACT;
	}
	else if(m_fFlying && m_fBoost)
	{	// Select boost animation
		switch((int)pev->vuser1[0])
		{
		default:
		case BOOST_FRONT:
			animDesired = LookupSequence( "forward_charge" );
			break;
		case BOOST_BACK:
			animDesired = LookupSequence( "backward_charge" );
			break;
		case BOOST_LEFT:
			animDesired = LookupSequence( "left_charge" );
			break;
		case BOOST_RIGHT:
			animDesired = LookupSequence( "right_charge" );
			break;
		case BOOST_UP:
			animDesired = LookupSequence( "up_charge" );
			break;
		case BOOST_DOWN:
			animDesired = LookupSequence( "down_charge" );
			break;
		}
	...
	{	// Saber handler
		...
		else if(iterAnim->eAnimType == ANIMTYPE_FULLBODY)
		{	animDesired = m_Anim.SetAnimation(iterAnim);
		}
		// We have a non-fullbody animation sequence, but a fullbody anim is overwritting. Delete the non-fb anim
		else if(iterAnim != NULL && animDesired)
		{	m_Anim.RemoveAnimation( iterAnim );
		}
	}
...
* Get Weapon Idle
*/
int CBasePlayer::GetWeaponIdle( void )
{	int	iWeaponAnim = 0;
	char szWeaponAnim[64];

	sprintf(szWeaponAnim,"%s_idle", m_szAnimExtention);
	iWeaponAnim = LookupSequence(szWeaponAnim);
...
*/
int CBasePlayer::UpperBodyAnimation( std::list<entanim_s>::iterator iterAnim )
{	int animDesired = 0;
	int iWeaponAnim = GetWeaponIdle();

	// Flying
	if(m_fFlying)
	{	if(iWeaponAnim)
		{	animDesired = iWeaponAnim;
		}
		else
		{	animDesired = LookupSequence( "fly_idle" );
		}
	...
	{	if(pev->velocity.z > 0)
		...
		{	animDesired = LookupSequence( "fall" );
		}
	...
	{	float flSpeed = pev->velocity.Length();
		if((pev->button & IN_FORWARD) || (pev->button & IN_MOVELEFT) || (pev->button & IN_MOVERIGHT))
		{	if(flSpeed > 100)
			{	animDesired = LookupSequence( "run_forward" );
			}
			else
			{	animDesired = LookupSequence( "walk_forward" );
			}
		}
		else if(pev->button & IN_BACK)
		{	if(flSpeed > 100)
			...
			{	animDesired = LookupSequence( "walk_backward" );
			}
		}
		else
		{	if(flSpeed > 10)
			...
			{	if(iWeaponAnim)
				...
				{	animDesired = LookupSequence( "deep_idle" );
				}
			}
		}
	...
	if(iterAnim != NULL)
	{	if(((iterAnim->eAnimType == ANIMTYPE_UPPERGAIT) || (iterAnim->eAnimType == ANIMTYPE_UPPERONLY)))
		{	animDesired = m_Anim.SetAnimation(iterAnim);
		}
	}
...
* Gait Animation
*/
int CBasePlayer::GaitAnimation( std::list<entanim_s>::iterator iterAnim )
{	int animDesired = 0;
	...
	if(iterAnim != NULL && iterAnim->eAnimType == ANIMTYPE_UPPERGAIT)
	{	// Set Sequence
		animDesired = LookupSequence( iterAnim->strAnim[iterAnim->iCurAnim].c_str() );
	}

	return animDesired;
}

// Only used for death animations
void CBasePlayer::SetAnimation( Activity Activity )
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	// Edited by Pcjoe, play full death anims
	if (pev->framerate && !m_fSequenceFinished)
	{	StudioFrameAdvance( );
		return;
	}
	else if(pev->framerate && m_fSequenceFinished)
	{	StopAnimation();
		pev->effects |= EF_NOINTERP;
		pev->framerate = 0.0;
	}

	// Pcjoe: Min Respawn Time
	if(m_flKilled + respawntime.value > gpGlobals->time)
	{	return;
	}
...
// Removed by Pcjoe, replaced by newer spectating system
/*
...
void CBasePlayer::PreThink(void)
{
	...
	// Pcjoe: Robin Walker - Observer Button Handling
	if ( IsObserver() )
	{
	...
	// Pcjoe: Check for double jump
	if(m_fDoubleJump != pev->iuser3)
	{
		m_fDoubleJump = pev->iuser3;
		if(m_fDoubleJump && !m_fFlying)
		{	if(m_pBoost)
			{	m_pBoost->Remove();
				m_pBoost = NULL;
			}
			EMIT_SOUND(ENT(pev), CHAN_BODY, "player/boost_doublejump.wav", 1, ATTN_NORM); // Emit sound
			m_pBoost = CBoost::CreateBoost(this, TRUE);
		}
	}

	// Pcjoe: Check for impact, but dont bother with it while we're shooting freedoms attacks
	if(!m_fFreedomBlast)
	{	if(pev->fuser4 && !m_fImpact)
		{	m_fImpact = TRUE;
			// Emit Sound
			EMIT_SOUND(ENT(pev), CHAN_BODY, "player/hit_ground.wav", 1, ATTN_NORM);
			m_flSlowDown = 0;
		}
		else if(!pev->fuser4 && m_fImpact)
		{	m_fImpact = FALSE;
			m_Anim.RemoveAllAnims();
			m_flSlowDown = 1;
		}
	}
	else
	{	// Restore movement and remove anims
		if(m_iMissles == 0)
		{	m_fFreedomBlast = FALSE;
			m_flSlowDown = 1.0;
			m_Anim.RemoveAllAnims();
			UTIL_LockModelAngles( (CBaseEntity*)this, FALSE );
		}
	...
	// Pcjoe: Handle saber melee
	if(m_pClass && m_pClass->ClassID() != PC_NONE)
	{	m_Saber.ManageSaber();
	}
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			{//splat
				...
				// Removed by Pcjoe, no "splat" sounds
	...
	// added by jason update player boxes
	//do this before we clear
	//ALERT(at_console, "Every Frame\n" );
	g_SaberPhys.UpdatePlayers( this );
	// calls this more than needed but it doesnt matter
	g_SaberPhys.UpdatePhysics( );
	// Added by Pcjoe, calculate saber collisions
/*	if(m_Saber.RunCollision())
		m_Saber.Collision();*/
...
pt_end:
    ...
    // Pcjo: Fix spinning dead bodies, by omega
	...
	if( m_iNextClass )
	{
		ChangeNumericalClass( m_iNextClass );
	}
...
/*	if (g_pGameRules->IsCoOp())
	...
	//else if( g_pGameRules->IsTeamplay( ) || g_pGameRules->IsSeige( ) )
	{
		// spawn the player at a random spawn point
		pSpot = g_pLastSpawn;
		for( int iCount = RANDOM_LONG( 1, 5 ); iCount > 0; iCount -- )
		{
			pSpot = UTIL_FindEntityByClassname( pSpot, "info_team_spawn" );
			if( pSpot )
			{
				while( pSpot && pSpot->pev->team != pPlayer->pev->team )
				{
					//if teams dont match find matching teams
					pSpot = UTIL_FindEntityByClassname( pSpot, "info_team_spawn" );
				}
			}
		}
		...
		while( pSpot && pSpot == pFirstSpot )
		{
			if( pSpot )
			{
				// if the spot exists check team
				if( pSpot->pev->team == pPlayer->pev->team )
				{
					if( IsSpawnPointValid( pPlayer, pSpot ) )
					{
		...
		{
			CBaseEntity *ent = NULL;
			while( ( ent = UTIL_FindEntityInSphere( ent, pSpot->pev->origin, 36 ) ) != NULL )
			{
				if( ent->IsPlayer( ) && !( ent->edict( ) == player ) )
				{
					ent->TakeDamage( VARS( INDEXENT( 0 ) ), VARS( INDEXENT( 0 ) ), 300, DMG_GENERIC );
	...
	{
		pSpot = UTIL_FindEntityByClassname(NULL, "info_team_spawn");
		if ( !FNullEnt(pSpot) )
			goto ReturnSpot;
	}
	else
...
// dont let uninitialized value here hurt the player
	...
	m_Anim.Reset();
	m_Saber.Reset();
	m_Anim.SetOwner( (CBaseAnimating*)this );
	m_Saber.SetOwner( this );
	// Added by Groovy
	m_iClientScore = -1;
	...
	// added by jason
	g_pGameRules->IsAllowedToSpawn( this );
	// added by jason
	m_iNextClass = 0;
	m_flRespawnTime = 0;
...
/*  SET_MODEL(ENT(pev), "models/player.mdl");
	...
	// test loading script by jason
	if( pev->team != TEAM_OBSERVER )
	{	g_SaberPhys.CreatePlayerBox( this );
		g_SaberPhys.CreateSaberBox( this );
...
//    g_ulModelIndexPlayer = pev->modelindex;
	...
	{
		UTIL_SetSize(pev, VEC_HULLS_MIN, VEC_HULLS_MAX);
	}
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 102:
		// Gibbage!!!
		CGib::SpawnRandomGibs( pev, 1, 1, 0 );
		break;
...
void CBasePlayer :: UpdateClientData( void )
{
	if (m_fInitHUD)
	{
		...
		if ( !m_fGameHUDInitialized )
		{
			...
			MESSAGE_END();
			// added by jason particle msgs 1-9-03
			CBaseEntity *pBrush = NULL;
			pBrush = UTIL_FindEntityByClassname( pBrush, "particle_brush");

			while( pBrush )
			{
				CPartBrush *pPartBrush = ( CPartBrush *)pBrush;

				pPartBrush->MsgClient( this );

				pBrush = UTIL_FindEntityByClassname( pBrush, "particle_brush");
			}

			pBrush = NULL;
			pBrush = UTIL_FindEntityByClassname( pBrush, "particle_point");
			while( pBrush )
			{
				CPartPoint *pPartPoint = ( CPartPoint *)pBrush;

				pPartPoint->MsgClient( this );
	...
	// Pcjoe: Update speed to client
	int iSpeed = GetMaxSpeed();
	if(iSpeed != m_iClientSpeed)
	{	// Set speed and send to client
		g_engfuncs.pfnSetClientMaxspeed( ENT( pev ), iSpeed );
		pev->iuser4 = iSpeed;
		m_iClientSpeed = iSpeed;
	}

	// Pcjoe: Update power to client 
	if(m_flPower != m_flClientPower)
	{	m_flClientPower = m_flPower;
		MESSAGE_BEGIN( MSG_ONE, gmsgPower, NULL, pev );
			WRITE_COORD( m_flPower );
		MESSAGE_END();
	}

	// grOOvy: Update score to client
	if( m_iPlayerScore != m_iClientScore)
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgScore, NULL, pev );
			WRITE_LONG ( m_iPlayerScore );
		MESSAGE_END();
		m_iClientScore = m_iPlayerScore;
	}
	...
	// Removed by Pcjoe, no status bar in mechmod
	/*
	// Update Status Bar
