...
// Pcjoe: Entity By Index
CBaseEntity	*UTIL_EntityByIndex( int idx )
{	CBaseEntity *pEntity = NULL;

	if ( idx > 0 && idx <= MAX_ENTITY  )
	{	
...
// Pcjoe: Set Model
void UTIL_SetModel( const char *pModel, entvars_t *pev )
{	if(pModel)
	{	pev->modelindex = MODEL_INDEX(pModel);
		pev->model = MAKE_STRING(pModel);
	}
	else
	{	pev->modelindex = 0;
		pev->model = 0;
...
// Pcjoe: Set Player Model
void UTIL_SetPlayerModel( const char *pModel, entvars_t *pev )
{	char szModelName[256];
	sprintf(szModelName,"models/player/%s/%s.mdl", pModel, pModel);
	UTIL_SetModel( szModelName, pev );
}

// Pcjoe: Lock Model Angles
extern int gmsgLockMdl;
void UTIL_LockModelAngles( CBaseEntity *pEnt, BOOL fLock )
{	MESSAGE_BEGIN( MSG_ONE, gmsgLockMdl, NULL, pEnt->edict() );
