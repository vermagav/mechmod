...
{
	// added by jason first thing called on map change
	g_iHullsFixed = 0;
	g_SaberPhys.KillPhysics( );
	g_SaberPhys.InitPhysics( );
	g_fGameOver = FALSE;
	Precache( );
...
// sounds used from C physics code
	...
	// added by grOOvy: 14/11/04

	// All the head models
	PRECACHE_MODEL("models/gibs/vf1j/head.mdl");
	PRECACHE_MODEL("models/gibs/af49/head.mdl");
	PRECACHE_MODEL("models/gibs/ds/head.mdl");
	PRECACHE_MODEL("models/gibs/mk2/head.mdl");
	PRECACHE_MODEL("models/gibs/freedom/head.mdl");
	
	// All the part models
	PRECACHE_MODEL("models/gibs/vf1j/part1.mdl");
	PRECACHE_MODEL("models/gibs/vf1j/part2.mdl");
	PRECACHE_MODEL("models/gibs/vf1j/part3.mdl");
	PRECACHE_MODEL("models/gibs/vf1j/part4.mdl");
	PRECACHE_MODEL("models/gibs/vf1j/part5.mdl");
	PRECACHE_MODEL("models/gibs/af49/part1.mdl");
	PRECACHE_MODEL("models/gibs/af49/part2.mdl");
	PRECACHE_MODEL("models/gibs/af49/part3.mdl");
	PRECACHE_MODEL("models/gibs/af49/part4.mdl");
	PRECACHE_MODEL("models/gibs/af49/part5.mdl");
	PRECACHE_MODEL("models/gibs/ds/part1.mdl");
	PRECACHE_MODEL("models/gibs/ds/part2.mdl");
	PRECACHE_MODEL("models/gibs/ds/part3.mdl");
	PRECACHE_MODEL("models/gibs/ds/part4.mdl");
	PRECACHE_MODEL("models/gibs/ds/part5.mdl");
	PRECACHE_MODEL("models/gibs/mk2/part1.mdl");
	PRECACHE_MODEL("models/gibs/mk2/part2.mdl");
	PRECACHE_MODEL("models/gibs/mk2/part3.mdl");
	PRECACHE_MODEL("models/gibs/mk2/part4.mdl");
	PRECACHE_MODEL("models/gibs/mk2/part5.mdl");
	PRECACHE_MODEL("models/gibs/freedom/part1.mdl");
	PRECACHE_MODEL("models/gibs/freedom/part2.mdl");
	PRECACHE_MODEL("models/gibs/freedom/part3.mdl");
	PRECACHE_MODEL("models/gibs/freedom/part4.mdl");
	PRECACHE_MODEL("models/gibs/freedom/part5.mdl");
	...
	// Added by Pcjoe, parse script(s)
	g_MMParse.ParseScript( "melee/mmscript.txt" );
	// Precache elements
	std::vector<std::string>::iterator iterPrecache;
	ForEachIn( iterPrecache, g_MMParse.m_Precache )
	{	// save string and string length
		std::string strPrecache = *iterPrecache;
		int iSize = strPrecache.size()-1; 
		if(iSize < 4)
		{	ALERT(at_console,"INVALID PRECACHE: %s\n", strPrecache.c_str());
			continue;
		}

		// Precache sounds
		if(strPrecache.find(".wav") != -1)
		{	int iSound = ALLOC_STRING( strPrecache.c_str() );
			PRECACHE_SOUND( (char*)STRING( iSound ) );
//			ALERT(at_console,"Precached \"%s\"\n", strPrecache.c_str());
		}
		// Precache models
		else if(strPrecache.find(".mdl") != -1)
		{	std::string strModel = "models/melee/" + strPrecache;
			int iModel = ALLOC_STRING( strModel.c_str() );
			PRECACHE_MODEL( (char*)STRING( iModel ) );
//			ALERT(at_console,"Precached \"%s\"\n", strModel.c_str());
		}
		// Not a valid precache
		else
		{	ALERT(at_console,"INVALID PRECACHE: \"%s\"\n", strPrecache.c_str());
		}
	}
