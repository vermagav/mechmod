...
// Added by Pcjoe: Acceleration
#define ACCEL_DEFAULT	10	// Default acceleration
#define ACCEL_WALK		5	// Mech walk acceleration
...
// Added by Pcjoe
#define PLAYER_IMPACT_SPEED	750
...
void PM_FixHulls( void );
int g_iHullsFixed = 0;

// Pcjoe: Store velocity of player clientside for later use
#ifdef CLIENT_DLL
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	// Added by Pcjoe
	default:
		pmove->PM_PlaySound( CHAN_BODY, "player/stomp2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );
		break;
	// Removed by Pcjoe
	/*
	default:
...
void PM_UpdateStepSound( void )
{
		...
		( speed >= velwalk || !pmove->flTimeStepSound ) )
	{
		...
		// Added by Pcjoe, longer timestepsounds
		pmove->flTimeStepSound = 500;
...
//
	...
	// Added by Pcjoe, slow down acceleration for walk
	pmove->movevars->accelerate = ACCEL_WALK;
	...
	// Added by Pcjoe, restore to default acceleration
	pmove->movevars->accelerate = ACCEL_DEFAULT;
...
// Added by Rick, edited by Pcjoe
void PM_PlayerFly(void)
{
	int		i;
	vec3_t	wishvel;
	float	wishspeed;
...
//
	...
	// Sinking after no other movement occurs
	if (!pmove->cmd.forwardmove && !pmove->cmd.sidemove && !pmove->cmd.upmove)
		wishvel[2] --;		// drift towards bottom
	else  // Go straight up by upmove amount.
		wishvel[2] += pmove->cmd.upmove;
...
// Added by Pcjoe, Player boost
void PM_PlayerBoost(void)
{
	int		i;
	vec3_t	wishvel;
	float	wishspeed;
...
//
	// Get boost direction
	switch((int)pmove->vuser1[0])
	{	
	default:
	case BOOST_FRONT:
		for(i=0;i<3;i++)
			wishvel[i] = pmove->forward[i] * pmove->iuser4;
		break;
	case BOOST_BACK:
		for(i=0;i<3;i++)
			wishvel[i] = -pmove->forward[i] * pmove->iuser4;
	...
	case BOOST_LEFT:
		for(i=0;i<3;i++)
			wishvel[i] = -pmove->right[i] * pmove->iuser4;
		break;
	case BOOST_RIGHT:
		for(i=0;i<3;i++)
			wishvel[i] = pmove->right[i] * pmove->iuser4;
	...
	case BOOST_UP:
		for(i=0;i<3;i++)
			wishvel[i] = pmove->up[i] * pmove->iuser4;
		break;
	case BOOST_DOWN:
		for(i=0;i<3;i++)
			wishvel[i] = -pmove->up[i] * pmove->iuser4;
...
{
	// Edited by Pcjoe, no control while in the air
	/*
	int			i;
	vec3_t		wishvel;
	float		fmove, smove;
...
//	qboolean cansuperjump = false;
	...
	// Added by Pcjoe, double jump
	if(pmove->iuser3 == 0 && pmove->onground == -1 && pmove->movetype == MOVETYPE_WALK)
	{	if ( !(pmove->oldbuttons & IN_JUMP) && pmove->fuser3 > 100 )
		{	pmove->fuser3 = pmove->fuser3 - 100;

			// Get boost jump direction
			VectorCopy( pmove->forward, vecForward );
			VectorCopy( pmove->right, vecRight );
			// terminate Z
			vecForward[2]=0;
			vecRight[2]=0;
			// Normalize
			...
			// Add directions to double jump velocity
			for(i=0;i<3;i++)
				vecDblJump[i] = vecForward[i] * pmove->cmd.forwardmove + vecRight[i] * pmove->cmd.sidemove;

			// Get real velocity
			VectorNormalize(vecDblJump);
			for(i=0;i<3;i++)
				vecDblJump[i] = vecDblJump[i] * 300;
			vecDblJump[2] = 400;

			// Set boost jump direction
			VectorCopy( vecDblJump, pmove->velocity );
...
//	pmove->velocity[2] = sqrt(2 * 2300 * 45.0);
	pmove->velocity[2] = sqrt(2 * 1600 * 45.0);
...
{
	// Added by Pcjoe, land imprint
	if( pmove->onground != -1 && !pmove->dead && pmove->flFallVelocity >= PLAYER_IMPACT_SPEED )
	{	pmove->fuser4 += 750;
	}
	// Removed by Pcjoe
	/*
...
{	int i=0;
	...
	// Added by Pcjoe, no step sounds in flight
	if(pmove->movetype == MOVETYPE_WALK)
	{	PM_UpdateStepSound();
	}
...
//	PM_Duck();
	...
	if ( !pmove->dead && !(pmove->flags & FL_ONTRAIN) )
	{
		if ( pLadder )
		{
			PM_LadderMove( pLadder );
			      ...
			      pmove->movetype != MOVETYPE_FLY &&
				  // Added by Pcjoe
				  pmove->movetype != MOVETYPE_PLAYERFLY &&
				  pmove->movetype != MOVETYPE_PLAYERBOOST)
		{
	...
	// Added by Rick, edited by Pcjoe
	case MOVETYPE_PLAYERFLY:
		// TODO: Check for falling damage
	...
	// Added by Pcjoe, Boost mode
	case MOVETYPE_PLAYERBOOST:
		if (!(pmove->cmd.buttons & IN_JUMP))
		{	pmove->oldbuttons &= ~IN_JUMP;
		}
	...
	// Pcjoe: Copy player velocity to the client
#ifdef CLIENT_DLL
	for(i=0;i<3;i++)
	{	vecPmVelocity[i] = pmove->velocity[i];
	}
#endif
}
...
void PM_UpdateClipBox( void )
{
	   vec3_t vMechMax, vMechMin, vPoint;
	   vMechMax[0] = vMechMax[1] = PLAYER_WIDTH; vMechMax[2] = PLAYER_HEIGHT;
	   vMechMin[0] = vMechMin[1] = PLAYER_WIDNEG; vMechMin[2] = PLAYER_HEINEG;
	   vPoint[0] = vPoint[1] = vPoint[2] = 0;
	   
	   VectorCopy( vMechMax, pmove->player_maxs[0] );
	   VectorCopy( vMechMin, pmove->player_mins[0] );
	   VectorCopy( vMechMax, pmove->player_maxs[1] );
	   VectorCopy( vMechMin, pmove->player_mins[1] );
	   VectorCopy( vPoint, pmove->player_maxs[2] );
	   VectorCopy( vPoint, pmove->player_mins[2] );
	 //  VectorCopy( vMechMax, pmove->player_maxs[3] );
	 //  VectorCopy( vMechMin, pmove->player_mins[3] );
}

model_t *g_pModel = NULL;
void PM_FixHulls( void )
{
/*	int iModel = 0;
	vec3_t vMechMax, vMechMin, vPoint;
   ...
   if( !g_pModel )
   {
	   g_pModel = pmove->physents[0].model;
   }

    VectorCopy( vMechMax, pmove->physents[0].model->hulls[2].clip_maxs );
	VectorCopy( vMechMin, pmove->physents[0].model->hulls[2].clip_mins );
	VectorCopy( vMechMax, pmove->physents[0].model->hulls[1].clip_maxs );
	VectorCopy( vMechMin, pmove->physents[0].model->hulls[1].clip_mins );
	VectorCopy( vPoint, pmove->physents[0].model->hulls[0].clip_maxs );
	VectorCopy( vPoint, pmove->physents[0].model->hulls[0].clip_mins );
//	VectorCopy( vMechMax, pmove->physents[0].model->hulls[3].clip_maxs );
	//VectorCopy( vMechMin, pmove->physents[0].model->hulls[3].clip_mins );

	for( iModel = 0; iModel < 1024; iModel++ )
	{
		if( pmove->physents[0].model[iModel].name[0] == '*' )
		{
			VectorCopy( vMechMax, g_pModel[iModel].hulls[2].clip_maxs );
			VectorCopy( vMechMin, g_pModel[iModel].hulls[2].clip_mins );
			VectorCopy( vMechMax, g_pModel[iModel].hulls[1].clip_maxs );
			VectorCopy( vMechMin, g_pModel[iModel].hulls[1].clip_mins );
			VectorCopy( vPoint, g_pModel[iModel].hulls[0].clip_maxs );
			VectorCopy( vPoint, g_pModel[iModel].hulls[0].clip_mins );
		//	VectorCopy( vMechMax, g_pModel[iModel].hulls[3].clip_maxs );
		//	VectorCopy( vMechMin, g_pModel[iModel].hulls[3].clip_mins );
